...
"谛观 GreenwashGuard" 是一个面向**银行绿色信贷贷后管理**、**绿色债券存续期管理**等机构的企业漂绿风险监测工具。系统通过三个异构大语言模型（DeepSeek-R1-32B、Qwen-3-32B、OpenPangu-Pro-MoE-72B）对企业年报 MD&A / ESG 报告做**逐句分类**（实质性陈述 / 描述性陈述 / 非环保语句），多数投票确权，结合**语境情感打分**与**行业基准修正**，合成 **"GW 指数"** 衡量漂绿风险。
...
| 🤖**三模型集成** | DeepSeek-R1 + Qwen-3 + OpenPangu 独立推理 + 多数投票融合     |
| 🎭**Mock 模式**  | 无需 API Key 即可演示完整功能，使用固定示例数据模拟 API 返回 |
...
├─ Qwen-3-32B（中文专家）
└─ OpenPangu-Pro-MoE-72B（知识专家）
    ↓
多数投票确权（Fleiss' Kappa 一致性检验）
    ↓
语境情感打分（仅描述性语句）
    ↓
GW 指数 = 企业环境语调 - 行业年度中位数
    ↓
